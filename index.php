...
<?php
//se asegura si el usuario consta de unique_id el cual es un id unico para cada usuario
if (!isset($_SESSION['unique_id'])) {
    die("Error: Usuario no autenticado.");
}

$unique_id = $_SESSION['unique_id'];
//esta consulta selecciona varios datos de users que es la base de datos donde todos los usuarios estan
...
//inserta los datos en la tabla publications donde estan las publicaciones principales de la pagina
        $sql = "INSERT INTO publications (usuario, unique_id, titulo, mensaje, imagen, video, fecha) VALUES ('$autor','$unique_id', '$titulo', '$mensaje', '$imagen', '$video', NOW())";
...
<div class="mensajes">
    <div class="cop">
        <?php
        //selecciona ciertos datos de las tablas publications y users y los muestra al usuario
        $sql = "SELECT p.*, u.img FROM publications p JOIN users u ON p.unique_id = u.unique_id ORDER BY p.fecha DESC";
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if (!empty($row["imagen"])) {
                    echo '<img src="' . $row["imagen"] . '" alt="Imagen del mensaje" style="max-width: 50%; height: auto;"><br>';
                }
                if (!empty($row["video"])) {
